    ...
    public partial class MainUpdater : Form
    {
        ...
        private async void MainUpdater_Load(object sender, System.EventArgs e)
        {
            ...
            if (File.Exists(LocalAppData + "/NewVer.txt"))
            {
                ...
                {
                    MessageBox.Show("Is not an administrator. Please restart and click OK on the UAC Prompt. Error: PV003", "Updater for PhotoViewer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Application.Exit();
                }
            }
            else
            {
                MessageBox.Show($"Could not find NewVer.txt in {Directory.GetCurrentDirectory()}. This file tells the updater what version of Photo Viewer software to download. Error: PV004", "Updater for PhotoViewer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                Application.Exit();
            }
        }
