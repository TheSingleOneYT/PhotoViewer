...
namespace PhotoViewer
{
    public partial class Main : Form
    {
        ...
        { 
            ...
            foreach (var process in Process.GetProcessesByName("PhotoViewer"))
            {
                i++;
            }
            ...
            if (i > 1)
            {
                MessageBox.Show("Only one Photo Viewer process can be opened and running at once. Error: PV001", "Photo Viewer", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void MainImage_Click(object sender, EventArgs e)
        {
            if (MainImage.Image == null)
            {
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    EditCMS.Items[0].Enabled = true;
                    EditCMS.Items[1].Enabled = true;
                    EditCMS.Items[2].Enabled = true;
                    EditCMS.Items[3].Enabled = true;
                    EditCMS.Items[4].Enabled = true;

                    MainImageContextStrip.Items[1].Enabled = true;
                    MainImageContextStrip.Items[2].Enabled = true;
                    FileCMS.Items[1].Enabled = true;
                    FileCMS.Items[2].Enabled = true;
                }
            }
            else
            {
        ...
        {
            DialogResult dr = MessageBox.Show("This will make the image lose transparency. Continue?", "Photo Viewer", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
        ...
        private void resetImageToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (Form.ActiveForm.Text.EndsWith("*"))
            {
                ...
                {
                    MessageBox.Show("This image was pasted or dragged in and dropped. This means the original file cannot be found. Error: PV002", "PhotoViewer", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
            else
            {
